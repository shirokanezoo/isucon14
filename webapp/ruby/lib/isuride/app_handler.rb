...
module Isuride
  class AppHandler < BaseHandler
    ...
    get '/nearby-chairs' do
      ...
      response = db_transaction do |tx|
        chairs = tx.query(
          'SELECT chairs.*, chair_locations2.latitude as latitude, chair_locations2.longitude as longitude FROM chairs LEFT OUTER JOIN chair_locations2 ON chairs.id = chair_locations2.id WHERE chairs.is_busy = FALSE and chairs.is_active = TRUE'
        )
